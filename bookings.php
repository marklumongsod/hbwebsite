  ...
  <div class="container">
    <div class="row">
      ...
      <div class="col-12 my-5 px-4">
        <h2 class="fw-bold">ROOM BOOKINGS</h2>
    ...
    <div class="row">
      ...
      while ($data = mysqli_fetch_assoc($result)) {
        ...
        if ($data['booking_status'] == 'booked') {
          $status_bg = "bg-success";
          if ($data['arrival'] == 1) {
            $btn = "<a href='generate_pdf_pool.php?gen_pdf&id=$data[booking_id]' class='btn btn-dark btn-sm shadow-none'>Download PDF</a>";
        ...
        if (($data['arrival'] == 1 || $data['arrival'] == 0) && $data['booking_status'] == 0) {
          $status_text = 'Booked';
        } elseif ($data['booking_status'] == 'cancelled') {
          $status_text = 'Cancelled';
        } else {
          $status_text = $data['arrival'];
        }

        echo <<<bookings
          <div class='col-md-4 px-4 mb-4'>
            <div class='bg-white p-3 rounded shadow-sm'>
              <h5 class='fw-bold'>{$data['pool_name']}</h5>
              <p>₱{$data['price']}</p>
              <p>
                <b>Check-in-date: </b> {$checkin} <br>
                <b>Check-in-time: </b> {$checkin_time}
              </p>
              <p>
                <b>Amount: </b> ₱{$data['price']} <br>
                <b>Order ID: </b> {$data['order_id']} <br>
