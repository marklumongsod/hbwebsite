...
if (isset($_POST['add_room'])) {
  ...
  $q1 = "INSERT INTO `rooms` (`name`, `room_no`, `description`, `rate_3hrs`, `rate_6hrs`, `rate_12hrs`) VALUES (?,?,?,?,?,?)";
  $values = [
    $frm_data['name'],
    $frm_data['room_no'],
...
if (isset($_POST['get_all_rooms'])) {
  ...
  while ($row = mysqli_fetch_assoc($res)) {
    ...
    if ($row['isAvailable'] == 1) {
      $availability = "<span class='text-primary'>Available</span>";
    } else {
      $availability = "<span class='text-danger'>Not Available</span>";
    }
...
if (isset($_POST['edit_room'])) {
  ...
  $q1 = "UPDATE `rooms` SET `name`=?, `room_no`=?, `description`=?, `rate_3hrs`=?, `rate_6hrs`=?, `rate_12hrs`=? WHERE `id`=?";
  $values = [
    $frm_data['name'],
    $frm_data['room_no'],
    $frm_data['description'],
